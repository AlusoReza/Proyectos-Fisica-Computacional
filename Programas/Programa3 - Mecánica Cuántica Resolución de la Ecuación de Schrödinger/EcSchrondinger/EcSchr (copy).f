      ...
      COMPLEX*16:: CC
      COMPLEX*16, DIMENSION(:), ALLOCATABLE:: ALPHA, A, GAMMA, XM, PM
      COMPLEX*16, DIMENSION(:), ALLOCATABLE:: ETM, DELTAX, DELTAP, XM2
      COMPLEX*16, DIMENSION(:), ALLOCATABLE:: PCHEI
      COMPLEX*16, DIMENSION(:,:), ALLOCATABLE:: PHI, V, PHINORM, BETA, B
      COMPLEX*16, DIMENSION(:,:), ALLOCATABLE:: Q, PROB, DPHIDX, P, EC
      COMPLEX*16, DIMENSION(:,:), ALLOCATABLE:: EP, ET, DPHIDX2
...
!     ------------------------------------------------------      
      ...
      ALLOCATE(XM(0:NC))
      ALLOCATE(PM(0:NC))
      ALLOCATE(DPHIDX(0:S-1,0:NC))
      ALLOCATE(P(0:S-1,0:NC))
      ALLOCATE(EC(0:S-1,0:NC))
      ALLOCATE(EP(0:S-1,0:NC))
      ALLOCATE(ET(0:S-1,0:NC))
      ALLOCATE(ETM(0:NC))   
      ALLOCATE(DPHIDX2(0:S-1,0:NC))
...
!     EL VALOR MEDIO DE LA POSICIÓN PARA CADA INSTANTE DE TIEMPO ES LA SUMA DE LA POSICIÓN DE LA ONDA POR LA PROBABILIDAD DE ENCONTRAR ESTA MISMA EN LA DICHA POSICIÓN.
      DO J = 0, NC
         ...
         XM(J) = 0.D0
         DO I = 0, S
            XM(J) = XM(J) + J*PROB(I,J)
            XM(J) = XM(J) + (J**2.D0)*PROB(I,J)            
         END DO   
      END DO
      
!     EL VALOR MEDIO DEL MOMENTO DEBEMOS PRIMERO CALCULAR LA DERIVADA CON RESPECTO DE LA POSICIÓN PARA CADA INSTANDE DE TIEMPO DE LA FUNCION DE ONDA. EL MOMENTO EN CADA INSTANTE DE TIEMPO Y POSICIÓN. Y FINALMENTE EL MOMENTO MEDIO COMO LA SUMA TOTAL PARA CADA INSTANTE DE TIEMPO DEL MOMENTO EN SU RESPECTIVA POSICIÓN.
      
      DO J = 0, NC
         DO I = 0, S-1
            DPHIDX(I,J) = PHINORM(I+1,J)-PHINORM(I,J)
         END DO   
      END DO
...
!     AHORA, LA ENERGÍA TOTAL DEL SISTEMA E O H, AMBAS SON LO MISMO. SE OBTIENE COMO LA SUMA DE LA ENERGÍA CINÉTICA Y POTENCIAL. SE VA A SEGUIR EL MISMO PROCEDIMIENTO QUE PARA EL MOMENTO.
      ...
      DO J = 0, NC
         DO I = 0, S-1
            DPHIDX2(I,J) = DPHIDX(I+1,J)-DPHIDX(I,J)
         END DO   
      END DO
...
!     ------------------------------------------------------      
      ...
      DO J = 0, NC
         WRITE(11,*) J, NORMA(J), XM(J), PM(J), ETM(J), PCHEI(J)
      END DO   
      CLOSE(11)
